...
class IngredientTest(TestCase):
    def setUp(self):
        ...
        self.litres = mommy.make('Unit', name='l', name_abbrev='l', type=Unit.TYPE.volume, system=Unit.SYSTEM.si)
        ...
        self.tbsp = mommy.make('Unit', name='tbsp', name_abbrev='tbsp', type=Unit.TYPE.volume,
        ...
        self.floz = mommy.make('Unit', name='fluid_ounce', name_abbrev='floz', type=Unit.TYPE.volume,
                               system=Unit.SYSTEM.imperial)
        ...
        self.water = mommy.make('Food', name='water', conversion_src_unit=self.litres, conversion_factor=1000)
    ...
    def test_formatted_amount_no_unit(self):
        ...
        self.assertEqual(ingredient.formatted_amount(1.),
                         "1")
    ...
    def test_formatted_food_plural_just1(self):
        ...
        self.assertEqual(ingredient.formatted_food(),
                         "egg")
    ...
    def test_formatted_grams_mL(self):
        ingredient = mommy.make('Ingredient', amount=101.0, food=self.water, unit=self.mL)
        self.assertEqual(ingredient.formatted_grams(1.),
                         "(101 g)")
    ...
    def test_prep_method_None(self):
        ...
        self.assertEqual(ingredient.formatted_prep(),
                         "")
    ...
    def test_formatted_amount_scale(self):
        ingredient = mommy.make('Ingredient', amount=1, food=self.sugar, unit=self.cup)
        self.assertEqual(ingredient.formatted_amount(scale=2),
                         '2 cups')
    ...
    def test_formatted_amount_pinch(self):
        ...
        self.assertEqual(ingredient.formatted_amount(1.), 'pinch of')

    def test_formatted_amount_eight_tsp(self):
        ingredient = mommy.make('Ingredient', amount=0.125, unit=self.tsp)
        self.assertEqual(ingredient.formatted_amount(1.), '<sup>1</sup>&frasl;<sub>8</sub> tsp')
    ...
    def test_floats(self):
        ...
        self.assertEqual(ingredient.formatted_amount(2.), '1 <sup>1</sup>&frasl;<sub>3</sub> cup')
...
class TestFractions(TestCase):
    def test_to_nearest_frac(self):
        for maxdenom, x_and_expected in test_cases_nearest.items():
            for x, expected in x_and_expected:
                result = to_nearest_frac(x, maxdenom)
                self.assertEqual(result, expected)

    def test_to_frac_round_down(self):
        for maxdenom, x_and_expected in test_cases_round_down.items():
            for x, expected in x_and_expected:
...
class RecipeTest(TestCase):
    ...
    def test_get_text(self):
        ...
        mommy.make('Ingredient', amount='1', food=f, recipe=r, direction=d1)
        mommy.make('Ingredient', amount='1', food=f, recipe=r, direction=d1)
        mommy.make('Ingredient', amount='1', food=f, recipe=r, direction=d2)
        mommy.make('Ingredient', amount='1', food=f, recipe=r, direction=d2, prep_method=p)

        ret = r.get_text()
        expected = [('dir 1',
                     [('1', '(125 g)', 'food', '', 'detail'), ('1', '(125 g)', 'food', '', 'detail')]),
                    ('dir 2',
                     [('1', '(125 g)', 'food', '', 'detail'), ('1', '(125 g)', 'food', ', chopped', 'detail')])]
...
class UnitTest(TestCase):
    def test_unicode(self):
        u = mommy.make('Unit')
        self.assertEqual(str(u), u.name)


class FoodTest(TestCase):
    def test_unicode(self):
        f = mommy.make('Food')
        self.assertEqual(str(f), f.name_sorted)
...
class DirectionTest(TestCase):
    def test_unicode_title_too_long(self):
        ...
        self.assertEqual(str(d), 'abcdefghijklmnopqrstuvwxyzabcdefghijklmn...')
